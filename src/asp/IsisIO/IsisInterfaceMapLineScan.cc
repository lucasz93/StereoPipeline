...
// Constructor
IsisInterfaceMapLineScan::IsisInterfaceMapLineScan( const std::string &filename, boost::shared_ptr<Isis::Pvl> &label, boost::shared_ptr<Isis::Cube> &cube, boost::shared_ptr<Isis::Camera> &camera ) :
  IsisInterface( filename, label, cube, camera ){//, m_projection( Isis::ProjectionFactory::CreateFromCube(*m_label) ) {
...
public:
  ...
  inline EphemerisLMA(Isis::NaifContextPtr naif,
    ...
    m_naif(naif), m_point(point), m_camera(camera), m_distortmap(distortmap), m_focalmap(focalmap) {}
...
EphemerisLMA::result_type EphemerisLMA::operator()(EphemerisLMA::domain_type const& x) const {
  ...
  m_camera->setTime(Isis::iTime(x[0]), m_naif);
...
Vector2 IsisInterfaceMapLineScan::point_to_pixel(Vector3 const& point) const {
  ...
  m_camera->setTime(Isis::iTime(solution_e[0]), m_naif);
...
Vector3 IsisInterfaceMapLineScan::camera_center(Vector2 const& px) const {
  if (px != m_cache_px) {
    ...
    if (!m_groundmap->SetGround(m_naif,
                                Isis::Latitude(m_projection->UniversalLatitude(),
                                               Isis::Angle::Degrees),
