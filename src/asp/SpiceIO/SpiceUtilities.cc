...
namespace spice {
  ...
  double sclk_to_et(Isis::NaifContextPtr naif, std::string sclk, int naif_id) {
    ...
    naif->scs2e_c( naif_id, sclk.c_str(), &query_time );
    CHECK_SPICE_ERROR(naif);
    return query_time;
  }

  // Convert from Ephemeris time to UTC time.
  string et_to_utc(Isis::NaifContextPtr naif, double ephemeris_time) {
    char utc_time[18];
    naif->timout_c( ephemeris_time, "MM/DD/YYYY HR:MN:SC", 20, utc_time );
    CHECK_SPICE_ERROR(naif);
  ...
  void kernel_param(Isis::NaifContextPtr naif, std::string const& key, double &value) {
    SpiceInt nvalues;
    SpiceBoolean found;
    naif->gdpool_c(key.c_str(), 0, 1, &nvalues, &value, &found);
    if (!found) { vw_throw(LogicErr() << "spice::kernel_param() could not find value for key \"" << key << "\""); }
  }

  template <int ElemN>
  void kernel_param(Isis::NaifContextPtr naif, std::string const& key, Vector<double, ElemN> &value) {
    SpiceDouble elements[ElemN];
    SpiceInt nvalues;
    SpiceBoolean found;
    naif->gdpool_c(key.c_str(), 0, ElemN, &nvalues, elements, &found);
    if (!found) { vw_throw(LogicErr() << "spice::kernel_param() could not find value for key \"" << key << "\""); }
  ...
  template void kernel_param<2>(Isis::NaifContextPtr naif, std::string const& key, Vector<double, 2> &value);
  template void kernel_param<3>(Isis::NaifContextPtr naif, std::string const& key, Vector<double, 3> &value);
  template void kernel_param<4>(Isis::NaifContextPtr naif, std::string const& key, Vector<double, 4> &value);
  ...
  void body_state(Isis::NaifContextPtr naif, 
    ...
    naif->spkezr_c( spacecraft.c_str(),  time,  reference_frame.c_str(), "NONE", planet.c_str(), state, &light_time );
    ...
    naif->pxform_c( reference_frame.c_str(), instrument.c_str(), time, rotation_matrix);

    // Convert that matrix into a quaternion.
    SpiceDouble quaternion[4];
    naif->m2q_c( rotation_matrix, quaternion );
  ...
  void load_kernels(Isis::NaifContextPtr naif, std::list<std::string> &kernels) {
    ...
    naif->erract_c (  set, lenout, ret  );

    // Load the kernels
    list<string>::iterator iter;
    for (iter = kernels.begin(); iter != kernels.end(); iter++)
      naif->furnsh_c( (*iter).c_str() );
